...
const AllMovies = (props) => {
    ...
    const deleteMovie = (idFromBelow) => {
        axios
            .delete(`http://localhost:8000/api/movies/${idFromBelow}`)
            .then((res) => {
                console.log(res);
                console.log(res.data);
                setMovieList(
                    movieList.filter((movie) => movie._id !== idFromBelow)
                );
            })
            .catch((err) => console.log(err));
    };
    ...
    return (
        <div>
            ...
            {movieList.map((movie, index) => (
                <div style={{ textAlign: "center" }} key={movie._id}>
                    ...
                    <DeleteButton
                        deleteHandler={() => deleteMovie(movie._id)}
                    />
